...
$provinsi = $_POST['vprovinsi'];
$kota = $_POST['vkota'];
$kecamatan = $_POST['vkecamatan'];
$kelurahan = $_POST['vkelurahan'];
$alamat_lengkap = $_POST['valamat_lengkap'];
$id_kurir = '0';
...
//     $kota_d = $kota_data['city_name'];
    ...
    if ($data_lokasi_user) {
        $update_lokasi = $server->query("UPDATE `lokasi_user` SET `provinsi`='$provinsi', `kota`='$kota', `kecamatan`='$kecamatan', , `kelurahan`='$kelurahan', `alamat_lengkap`='$alamat_lengkap' WHERE `id_user`='$iduser'");
        $update_lokasi_invoice = $server->query("UPDATE `invoice` SET `provinsi`='$provinsi',`kota`='$kota', `kecamatan`='$kecamatan', `kelurahan`='$kelurahan', `alamat_lengkap`='$alamat_lengkap', WHERE `idinvoice`='$id_invoice'");
...
?>
        ...
        $insert_lokasi = $server->query("INSERT INTO `lokasi_user`(`id_user`, `provinsi`, `kota`, `kecamatan`, `kelurahan`, `alamat_lengkap`) VALUES ('$iduser', '$provinsi', '$kota', '$kecamatan', '$kelurahan', '$alamat_lengkap')");
        $update_lokasi_invoice = $server->query("UPDATE `invoice` SET `provinsi`='$provinsi',`kota`='$kota', `kecamatan`='$kecamatan', `kelurahan`='$kelurahan'`alamat_lengkap`='$alamat_lengkap'WHERE `idinvoice`='$id_invoice'");
